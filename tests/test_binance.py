...
def test_normalize_symbol():
    assert (
        BinanceProvider.normalize_symbol("BTC")
        == "BTCUSDT"
    )
    ...
    assert (
        BinanceProvider.normalize_symbol("BTCUSDT")
        == "BTCUSDT"
    )

    assert (
        BinanceProvider.normalize_symbol(" eth ")
        == "ETHUSDT"
    )
...
def test_validate_interval_accepts_supported():
    assert (
        BinanceProvider.validate_interval("15m")
        == "15m"
    )

    assert (
        BinanceProvider.validate_interval("1h")
        == "1h"
    )

    assert (
        BinanceProvider.validate_interval("4h")
        == "4h"
    )

    assert (
        BinanceProvider.validate_interval("1d")
        == "1d"
    )
...
def test_validate_limit_accepts_valid_values():
...
def test_parse_candle_rejects_invalid_ohlc_relationship():
    raw = make_kline(
        open_price=100,
        high_price=90,
        low_price=95,
