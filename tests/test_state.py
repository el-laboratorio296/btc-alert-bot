...
def test_store_records_last_update_time(tmp_path):
    ...
    assert data["updated_at"] == 1700001000.0


def test_store_rejects_invalid_path(tmp_path):
    with pytest.raises(
        TypeError
    ):
        StateStore(None)
...
def test_signal_metadata_is_preserved(tmp_path):
    store = StateStore(tmp_path / "state.json")
    ...
    assert loaded["technical_score"] == 82.0
    assert loaded["confidence"] == 91.0
    assert loaded["market_regime"] == "RISK_ON"
    assert loaded["risk_level"] == "LOW"


def test_signal_can_be_created_from_mapping():
    data = base_signal()

    signal = SignalState.from_dict(data)
...
def test_signal_from_mapping_normalizes_asset():
    data = base_signal(
        asset="btcusdt"
    )
...
def test_signal_from_mapping_requires_asset():
    data = base_signal()
    del data["asset"]

    with pytest.raises(ValueError):
        SignalState.from_dict(data)


def test_signal_from_mapping_requires_decision():
    data = base_signal()
    del data["decision"]
...
def test_store_can_save_mapping_via_signal_conversion(
    tmp_path,
):
    store = StateStore(tmp_path / "state.json")

    signal = SignalState.from_dict(
        base_signal()
    )
...
def test_state_file_is_not_modified_by_get(tmp_path):
    path = tmp_path / "state.json"
...
def test_state_store_handles_many_assets(tmp_path):
    store = StateStore(tmp_path / "state.json")

    for index in range(20):
        store.save_signal(
            SignalState(
                asset=f"COIN{index}USDT",
                decision="WAIT_CONFIRMATION",
                price=100.0 + index,
...
def test_clear_on_empty_store_is_safe(tmp_path):
    store = StateStore(tmp_path / "state.json")

    assert store.clear() == 0


def test_signal_age_never_becomes_negative(tmp_path):
    store = StateStore(
        tmp_path / "state.json",
        clock=lambda: 1000.0,
    )
...
def test_fresh_signal_at_exact_limit_is_fresh(tmp_path):
    store = StateStore(
        tmp_path / "state.json",
        clock=lambda: 1100.0,
    )
...
def test_negative_freshness_limit_is_rejected(tmp_path):
    store = StateStore(tmp_path / "state.json")
...
def test_state_round_trip_preserves_complete_signal(
    tmp_path,
):
    path = tmp_path / "state.json"
    ...
    assert loaded["bias"] == "BULLISH"
    assert loaded["price"] == 100000.0
    assert loaded["technical_score"] == 80.0
    assert loaded["confidence"] == 85.0
    assert loaded["market_regime"] == "RISK_ON"
    assert loaded["risk_level"] == "LOW"
    assert loaded["entry_zone"] == "100000-101000"
    assert loaded["invalidation"] == 98000.0
    assert loaded["stop"] == 97500.0
    assert loaded["tp1"] == 103000.0
    assert loaded["tp2"] == 106000.0
    assert loaded["tp3"] == 110000.0
    assert loaded["risk_reward"] == 2.0